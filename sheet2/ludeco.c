...
There is a chance that I did not get all the indices right. 
Especially when they have to be shifted to adjust them to a computer
counting from 0 instead of from 1. If the result is wrong, that is
likely the cause.
...
int main() {
  ...
  double *b = NULL; double *x = NULL; double *y = NULL;
  arr_alloc(&b, n); arr_alloc(&x, n); arr_alloc(&y, n);
  ...
  for (i1 = 0; i1 < n; i1++) {
    ...
    printf("%+f  %+f  %d\n", b[i1], 1/((double)i1+1), i1);
  }

  // do decomposition and solve equationa
