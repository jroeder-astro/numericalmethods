...
void gauss_seidel(vector<vector<double>> &y_n, vector<vector<double>> &y_next,
                  vector<vector<double>> &y_npo,  double Del, int size);
...
main() {
    ...
    vector<vector<double>> y_tmp(Ns+2, vector<double>(Ns+2));
    ...
    // initializing the temporary array with the analytic solution

    for (i2 = 0; i2 < Ns+2; i2++) {
        for (i3 = 0; i3 < Ns+2; i3++) {
            y_tmp[i2][i3] = analytic((double)i2*D, (double)i3*D);
        }
    }
    ...
    // i1 = 1;   //***
    // do {      //***
        gauss_seidel(y[i1], y_tmp, y[i1+1], D, Ns);

        // updating the new time level with the temporary array
        for (i2 = 1; i2 < Ns+1; i2++) {
            for (i3 = 1; i3 < Ns+1; i3++) {
                y[i1+1][i2][i3] = y_tmp[i2][i3];

                Ltemp = y_tmp[i2][i3] - nonlin((double)i2*D, (double)i3*D);
...
void gauss_seidel(vector<vector<double>> &y_n, vector<vector<double>> &y_next,
    ...
    // fill up temporary array with new solution

    for (int i = 1; i < size+1; i++) {
        for (int j = 1; j < size+1; j++) {
            y_next[i][j] = 0.25 * (y_n[i+1][j] + y_n[i][j+1] + y_npo[i-1][j] 
                + y_npo[i][j-1]) - pow((Del/2.),2.) * nonlin(i*Del, j*Del);
        }
    }
   
    // remnant from previous algorithm; here the gauss_seidel command in the
    // main funtion was evaluated at each point in space instead of having
    // the spacial loop within this function and evaluating the function 
